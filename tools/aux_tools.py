...
def add_palette_for_1ch_img(path, palette):
    '''
    !!! Функция предназначена для подготовки масок снимков к обучению с помощью MMSegmentation.
    Функция для создания палтиры у изображения с последующей перезаписью изображения. 

    path: путь к директории с картинками
